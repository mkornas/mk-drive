...
export function registerAdminRoutes(app: FastifyInstance, users: Users, locations: Locations): void {
  ...
  app.post<{ Body: { email?: unknown; name?: unknown; role?: unknown; password?: unknown; grants?: unknown } }>('/api/users', async (req, reply): Promise<User> => {
    admin(req);
    const b = req.body ?? {};
  ...
  app.patch<{ Params: { id: string }; Body: { name?: unknown; role?: unknown; disabled?: unknown; grants?: unknown; password?: unknown } }>('/api/users/:id', async (req): Promise<User> => {
    admin(req);
    ...
    users.audit({ userId: req.identity.id, email: req.identity.email, action: 'user.update', detail: { id, ...patch, grants: b.grants !== undefined, password: b.password !== undefined } });
    return users.get(id)!;
  });
